...
RSpec.describe 'Request Most Mentions', type: :request do

  describe 'GET /api/most_mentions' do
    ...
    context 'when there are tweets locaweb mention' do
      ...
      it 'returns all keys for tweets' do
        ...
        tweets_response_array.each do |tweet|
          tweet.each do |_key, tweet_field|
            expect(tweet_field.first).to have_key('screen_name')
            expect(tweet_field.first).to have_key('followers_count')
            expect(tweet_field.first).to have_key('retweet_count')
            expect(tweet_field.first).to have_key('favorite_count')
            expect(tweet_field.first).to have_key('text')
            expect(tweet_field.first).to have_key('created_at')
            expect(tweet_field.first).to have_key('profile_link')
            expect(tweet_field.first).to have_key('tweet_link')
          end
        end
